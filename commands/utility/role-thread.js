...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('role-thread')
		.setDescription('View only the Role Thread')
		.setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
		.addStringOption((option) =>
			option
				.setName('add-role')
				.setDescription('Add Role to the Thread whitelist.'))
		.addStringOption((option) =>
			option
				.setName('remove-role')
				.setDescription('Remove Role from Thread whitelist.'))
	...
	async execute(interaction) {
		...
		const threadFile = "./guild/role-thread.json";
		const guildOptions = "./guild/options.json";

		if (!fs.existsSync(threadFile)) {
			console.log(`${timer} CRITICAL : /thread.json file is missing.`);
			let a = [];
			console.log(`${timer} WARNING : Creating /role-thread.json.`);
			fs.writeFileSync(threadFile, encrypt(JSON.stringify(a)));
		}

		if (!fs.existsSync(threadFile)) {
			logs("CRITICAL", "return failed { ./guild/role-thread.json is missing }.");
			return await interaction.editReply(`\`./guild/thread.json\` is **Missing**\nPlease restart the bot`);
		};
		...
		let readThread = decrypt(fs.readFileSync(threadFile, "utf8"));
		let readOptions = decrypt(fs.readFileSync(guildOptions, "utf8"));

		let roleCurrent = JSON.parse(readThread);
		let option = JSON.parse(readOptions);

		function list() {
			let newReadRole = decrypt(fs.readFileSync(threadFile, "utf8"));
			let newRoleCurrent = JSON.parse(newReadRole);
			let result = "";
			let a = 0;
			for (let t of roleCurrent) {
				a+=1;
				result += a + ". <@&" + t + ">\n";
			};
			return "Role(s) thread :\n" + result;
		}
		...
		if (change === true) {
			fs.writeFileSync(threadFile, encrypt(JSON.stringify(roleCurrent)));
			logText = `<@${interaction.user.id}> Updated Role Thread`;
		...
		const embed = new EmbedBuilder()
			.setColor('008000')
			.setTitle('Media Thread Priviledged Role')
